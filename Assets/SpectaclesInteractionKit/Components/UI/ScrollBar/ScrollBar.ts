...
export class ScrollBar extends BaseScriptComponent {
    ...
    onAwake() {
        ...
        this.scrollViewScreenTransform =
            this.scrollViewSceneObject.getComponent(
    ...
    private setupInteractable() {
        ...
        interactable.onDragStart.bind((event) => {
    ...
    private sixDofDragUpdate(event: DragInteractorEvent) {
        ...
        if (
            ...
            if (
                this.scrollViewScreenTransform.worldPointToLocalPoint(
                    newDragPoint
                ).y >= 1
            ) {
    ...
    private localizeDragVector(dragVector: vec3): vec2 {
        ...
        const localXAxis = transform
            .getWorldRotation()
            .multiplyVec3(vec3.right());

        const localYAxis = transform.getWorldRotation().multiplyVec3(vec3.up());

        const localizedX =
            localXAxis.dot(dragVector) / transform.getWorldScale().x;
        const localizedY =
            localYAxis.dot(dragVector) / transform.getWorldScale().y;
    ...
    private scrollToEdge(topEdge: boolean) {
        ...
        this.scrollView.scrollBy(
            new vec2(0, -adjustedPercentage * this.overflow)
        );
    }
