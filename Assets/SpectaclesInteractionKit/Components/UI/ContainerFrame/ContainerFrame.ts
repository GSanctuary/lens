...
export class ContainerFrame extends BaseScriptComponent {
    ...
    onAwake() {
        ...
        this.closeButton.onTrigger.add(() => {
            ...
            this.onCloseButtonTrigger.invoke();
        });
        ...
        // SIK Components
        this.interactable = this.frame.createComponent(
        ...
        this.material.mainPass.frustumCullMode =
            FrustumCullMode.UserDefinedAABB;
        ...
        this.unSubscribeList.push(
            this.parentHoverBehavior.onHoverUpdate.add((e: InteractorEvent) => {
                ...
                while (
                    ...
                    targetParent = isNull(targetParent)
                        ? null
                        : targetParent.getParent();
                }

                const isNearFieldMode =
                    (e.interactor.inputType & InteractorInputType.BothHands) !==
                        0 && !(e.interactor as HandInteractor).isFarField();
        ...
        this.unSubscribeList.push(
            this.interactable.onTriggerUpdate((event: InteractorEvent) => {
                if (
                    event.interactor.targetHitInfo &&
    ...
    private showCursorHighlight = () => {
        ...
        animate({
            duration:
                CURSOR_HIGHLIGHT_ANIMATION_DURATION * (1 - startingHighlight),
    ...
    private scaleFrame = () => {
        ...
        this.material.mainPass.aspectRatio = new vec2(
            aspectRatio.x,
            aspectRatio.y
        );
        ...
        if (this.autoScaleContent) {
            if (!this.forcePreserveScale) {
                ...
                this.targetTransform.setLocalScale(
                    new vec3(factor.x, factor.y, 1)
                );
            } else {
    ...
    setIsFollowing = (isFollowing: boolean): void => {
        ...
        if (this.isFollowing) {
            if (this.billboardComponent !== null) {
                this.billboardComponent.xAxisEnabled =
                    (this.xOnTranslate && this.allowTranslation) ||
                    this.xAlways;
                this.billboardComponent.yAxisEnabled =
                    (this.yOnTranslate && this.allowTranslation) ||
                    this.yAlways;
            }
    ...
    private tweenMarginSqueeze = (
        ...
        animate({
            ...
            update: (t: number) => {
                this.onMovingShrinkFactor = lerp(
                    currentSqueeze,
                    targetSqueeze,
                    t
                );
